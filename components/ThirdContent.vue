<template>
  <div>
    <form class="flex flex-col justify-center" @submit.prevent="getTeamToGrent">
      <label class="text-2xl">input term</label>
      <input type="number" v-model="numberTeam" />
...
const numberTeam = ref(0)

const getTeamToGrent = () => {
  const newMatch = []
  const rounds = Math.ceil(Math.log(numberTeam.value) / Math.log(2))
  const bracketSize = Math.pow(2, rounds)
  const requiredByes = bracketSize - numberTeam.value

  console.log('Number of participants: ' + numberTeam.value)
  console.log('Number of rounds: ' + rounds)
  console.log('Bracket size:' + bracketSize)
  console.log('Required number of byes: ' + requiredByes)

  if (numberTeam.value < 2) {
    return []
  }
  ...
  for (let index = 1; index < rounds; index++) {
    const roundMatches = []
    const sum = Math.pow(2, index + 1) + 1

    for (let i = 0; i < matches.length; i++) {
      var home = changeIntoBye(matches[i][0], numberTeam.value)
      var away = changeIntoBye(sum - matches[i][0], numberTeam.value)
      roundMatches.push([home, away])
      home = changeIntoBye(sum - matches[i][1], numberTeam.value)
      away = changeIntoBye(matches[i][1], numberTeam.value)
      roundMatches.push([home, away])
    }
...
const changeIntoBye = (seed: any , numTeam: Number ) => {
    //return seed <= participantsCount ?  seed : '{0} (= bye)'.format(seed);  
    return seed <= numTeam ?  seed : null;
  }
</script>
